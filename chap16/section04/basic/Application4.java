package com.ohgiraffers.chap16.section04.basic;
...
public class Application4 {
    ...
    public static void main(String[] args) {
        ...
        List<String> highRatedActionMovies = movies.stream()
                .filter(m -> Objects.equals(m.getGenre(), "액션") && m.getRating() > 8)
                .map(Movie::getTitle)
                .toList();// 코드 작성

        // 2. 모든 영화의 평균 상영시간을 계산하여 반환
        double averageDuration = movies.stream()
                .collect(Collectors.averagingDouble(Movie::getDuration));// 코드 작성
